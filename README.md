...
本分支在原版基础上二次开发，新增了「歌单订阅下载」能力（详细设计见[需求文档](./LX_Music歌单订阅下载服务需求文档.md)）：

- **歌单订阅**：订阅音乐平台歌单（支持歌单 ID 或链接），按自定义周期检查新增歌曲，也可手动立即同步。
- **自动去重下载**：以「平台 + 歌曲 ID」为唯一键全局去重；开启 CD2 同步前先「扫描网盘已有歌曲」，把网盘已有歌曲登记到本地索引，避免重复下载和覆盖。
- **CD2 自动同步**：下载完成后自动写入元数据、封面和歌词，再复制到 CloudDrive2 挂载目录，通过 gRPC 确认上传成功，本地文件延迟 20 分钟清理。
...
开发与测试：`npm test` 运行回归测试套件（i18n 完整性、任务状态机、CD2 上传状态、清理安全等）；`node scripts/typecheck-vue.js <file.vue>` 对订阅相关组件做真实类型检查。
